...
/***/ function(module, exports, __webpack_require__) {
	...
	var HighOrderComposers = {
	  ...
	  composeRequiredProperty: function composeRequiredProperty(property, validatorFn, context) {
	    ...
	    return ['if (value.' + property + ' == null) {', '\terrors.push(new RAMLError(path.concat([\'' + property + '\']), ' + ERROR_MESSAGE + ', {name: \'' + property + '\'}));', '} else {', '\terrors = errors.concat(' + validatorFn + '(value.' + property + ', path.concat([\'' + property + '\'])));', '}'];
	  },
