...
/***/ (function(module, exports, __webpack_require__) {
	...
	var FACET_FRAGMENT_GENERATORS = {
	  ...
	  uniqueItems: function uniqueItems(value, context) {
	    ...
	    return ['if ((function() {', '\tvar valuesSoFar = Object.create(null);', '\tfor (var i = 0; i < value.length; ++i) {', '\t\tvar val = JSON.stringify(value[i]);', '\t\tif (val in valuesSoFar) {', '\t\t\treturn true;', '\t\t}', '\t\tvaluesSoFar[val] = true;', '\t}', '\treturn false;', '})()) {', '\terrors.push(new RAMLError(path, context, "ITEMS_UNIQUE"));', '}'];
	  },
