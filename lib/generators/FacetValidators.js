...
const FACET_FRAGMENT_GENERATORS = {
  ...
  uniqueItems: function(value, context) {
    throw Error('`uniqueItems` is not implemented');
  },
  ...
  items: function(value, context) {
    // let validatorFn = context.uses(value);
    // let ERROR_MESSAGE = context.getConstantString('ERROR_MESSAGES',
    //   'ITEMS_MAX', 'Must contain at most ${value} items in the array');

    // return [
    //   `if (value.find(function(value) {`,
    //     `\treturn ${validatorFn}(value, []).length !== 0;`,
    //   `}) !== -1) {`,
    //     `\terrors.push(new RAMLError(path, ${ERROR_MESSAGE}, {name: '${RAMLUtil.getTypeName(value)}'}));`,
    //   `}`
    // ];
    return [];
  },
